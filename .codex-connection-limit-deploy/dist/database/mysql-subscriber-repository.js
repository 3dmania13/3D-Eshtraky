...
export class MySqlSubscriberRepository {
    ...
    async getDevices(subscriber, limit) {
        ...
        ORDER BY last_seen_at DESC LIMIT ?`, [subscriber.username, subscriber.username, subscriber.username, limit]);
    ...
    async setDeviceSpeed(input) {
        ...
        if (input.selection === null) {
            await this.pool.execute("DELETE FROM subscriber_device_speed_limits WHERE username=? AND mac_address=?", [input.subscriber.username, existing.callingStationId]);
        }
        else {
            await this.pool.execute(`INSERT INTO subscriber_device_speed_limits
          (username,mac_address,selection,updated_at)
         VALUES (?,?,?,UTC_TIMESTAMP(6))
         ON DUPLICATE KEY UPDATE selection=VALUES(selection),updated_at=UTC_TIMESTAMP(6)`, [input.subscriber.username, existing.callingStationId, input.selection]);
        }
        return { ...existing, speedSelection: input.selection };
    }
    async invalidateDeviceSpeedApplications(username) {
        await this.pool.execute("DELETE FROM subscriber_device_speed_applications WHERE username=?", [username]);
    }
    ...
    async getConnectionLimit(username) {
        const [rows] = await this.pool.execute(`SELECT connection_limit
    ...
    async setConnectionLimit(input) {
        ...
        try {
            await connection.beginTransaction();
            await connection.execute(`INSERT INTO subscriber_connection_limits
          (username,connection_limit,updated_at)
         VALUES (?,?,UTC_TIMESTAMP(6))
         ON DUPLICATE KEY UPDATE
           connection_limit=VALUES(connection_limit),updated_at=UTC_TIMESTAMP(6)`, [input.username, input.limit]);
            await connection.execute("DELETE FROM radcheck WHERE username=? AND attribute='Simultaneous-Use'", [input.username]);
            await connection.execute(`INSERT INTO radcheck (username,attribute,op,value)
         VALUES (?, 'Simultaneous-Use', ':=', ?)`, [input.username, String(input.limit)]);
            await connection.commit();
        }
        catch (error) {
            await connection.rollback();
    ...
    mapDevice(username, row) {
        ...
        return {
            ...
            speedSelection: row.speed_selection === null ? null : String(row.speed_selection),
            callingStationId,
