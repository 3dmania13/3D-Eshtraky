...
export function registerRoutes(app, services) {
    ...
    app.get("/api/v1/subscriber/connection-limit", { preHandler: requireSubscriber }, async (request) => services.connectionLimits.get(subscriberPrincipal(request)));
    app.put("/api/v1/subscriber/connection-limit", {
        preHandler: requireSubscriber,
        config: {
            rateLimit: {
                max: 6,
    ...
    }, async (request) => services.connectionLimits.set(subscriberPrincipal(request), request.body.limit));
