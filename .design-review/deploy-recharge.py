...
live=Path('/var/www/html/3dradius/operators/nawa-card-recharge.php')
stage=Path('/home/anwar/recharge-design-stage.php')
old=live.read_bytes();new=stage.read_bytes()
assert hashlib.sha256(old).hexdigest()=='0cd09714ebf14ea2f2acf561dcec71ddaf9e53efbfa7e62740ffb2667978ecba', 'Concurrent change; stop'
...
before=state()
backup=Path('/home/anwar/ui-backups/recharge-design-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
backup.mkdir(mode=0o700);(backup/'nawa-card-recharge.php').write_bytes(old)
