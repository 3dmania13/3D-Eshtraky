...
<div class="nawa-page">
    ...
    <section class="nawa-stat-grid">
        <article class="nawa-stat"><span>إجمالي المستخدمين</span><strong><?= number_format($totalUsers) ?></strong></article>
        <article class="nawa-stat"><span>متصلون الآن</span><strong data-users-stat="online"><?= $statsReady ? number_format($onlineUsers) : '—' ?></strong></article>
        <article class="nawa-stat"><span>باقات مسددة هذا الشهر</span><strong data-users-stat="settled"><?= $statsReady ? number_format($settledThisMonth) : '—' ?></strong></article>
        <article class="nawa-stat"><span>منتهية الباقة</span><strong data-users-stat="expired"><?= $statsReady ? number_format($expiredUsers) : '—' ?></strong></article>
    </section>
    ...
    <section class="nawa-card subscriber-traffic-card" data-subscriber-traffic data-period-values='<?= nawa_e(json_encode($subscriberUsagePeriods, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)) ?>'>
        ...
        <div class="subscriber-traffic-grid"><article><span>إجمالي القيقات الموجودة</span><strong data-users-stat="quota"><?= $statsReady ? number_format((float) ($subscriberCurrent['quota_bytes'] ?? 0) / 1073741824, 2) . ' GB' : '—' ?></strong><small>مجموع باقات المشتركين الحالية</small></article><article><span>المصروف خلال الفترة</span><strong data-period-used><?= $statsReady ? number_format((float) ($subscriberUsagePeriods['day'] ?? 0) / 1073741824, 2) . ' GB' : '—' ?></strong><small data-period-label>اليوم</small></article><article><span>المتبقي الفعلي الآن</span><strong data-users-stat="remaining"><?= $statsReady ? number_format((float) ($subscriberCurrent['remaining_bytes'] ?? 0) / 1073741824, 2) . ' GB' : '—' ?></strong><small>بعد طرح استهلاك كل مشترك من باقته</small></article></div>
    </section>
