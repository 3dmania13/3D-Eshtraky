...
// Read-only display totals derived from the discovered wireless-device rows.
$wirelessTotal = count($rows);
$wirelessHasIp = 0;
$wirelessWithoutIp = 0;
foreach ($rows as $wirelessSummaryRow) {
    $wirelessIp = pick($wirelessSummaryRow, ['ip','ipaddress','nasname','host'], '');
    if (trim((string) $wirelessIp) !== '' && trim((string) $wirelessIp) !== '-') $wirelessHasIp++;
    else $wirelessWithoutIp++;
}
...
<link rel="stylesheet" href="static/css/wireless-reference.css?v=20260920">
<div class="wireless-page wireless-reference">
...
<section class="wireless-summary" aria-label="ملخص الأجهزة اللاسلكية">
    <article><i class="bi bi-phone"></i><span>إجمالي الأجهزة</span><strong><?=number_format($wirelessTotal)?></strong></article>
    <article><i class="bi bi-wifi"></i><span>لديها عنوان IP</span><strong><?=number_format($wirelessHasIp)?></strong></article>
    <article><i class="bi bi-clock-history"></i><span>بدون عنوان IP</span><strong><?=number_format($wirelessWithoutIp)?></strong></article>
    <article><i class="bi bi-slash-circle"></i><span>الأجهزة المحظورة</span><strong>—</strong></article>
</section>
