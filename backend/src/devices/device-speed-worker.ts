...
import type { LiveDeviceSpeedApplier } from "./live-device-speed-service.js";

interface PendingSpeedRule extends RowDataPacket {
  username: string;
  mac_address: string;
  selection: string;
  configured_at: Date;
...
export class DeviceSpeedWorker {
  ...
  async tick() {
    ...
    try {
      const [locks] = await connection.query<RowDataPacket[]>(
        "SELECT GET_LOCK(CONCAT('subscriber-device-speed:',DATABASE()),0) AS acquired",
      );
      locked = Number(locks[0]?.acquired) === 1;
      if (!locked) return;
      if (Date.now() >= this.nextCleanupAt) {
        await connection.execute(
          "DELETE FROM subscriber_device_speed_applications WHERE last_attempt_at<UTC_TIMESTAMP()-INTERVAL 30 DAY",
        );
        this.nextCleanupAt = Date.now() + 3_600_000;
      }
      const [rules] = await connection.query<PendingSpeedRule[]>(
        `SELECT DISTINCT limit_rule.username,limit_rule.mac_address,
      ...
      for (const rule of rules) {
        ...
        try {
          const result = await this.live.apply(
            String(rule.username),
            String(rule.mac_address),
            rate,
          );
          if (result.status === "applied") {
            await connection.execute(
              `UPDATE subscriber_device_speed_applications
      ...
      if (locked) {
        await connection.query(
          "SELECT RELEASE_LOCK(CONCAT('subscriber-device-speed:',DATABASE()))",
        );
      }
      connection.release();
    }
  ...
  private async recordAttempt(connection: PoolConnection, rule: PendingSpeedRule) {
    await connection.execute(
      `INSERT INTO subscriber_device_speed_applications
