...
import {
  requireSubscriber,
  subscriberPrincipal,
} from "../middleware/subscriber-auth.js";
...
export function registerRoutes(
  ...
  app.post<{ Body: { username: string; password: string } }>(
    ...
    async (request) =>
      services.auth.login(request.body.username, request.body.password),
  );
  ...
  app.get(
    "/api/v1/subscriber/usage/summary",
    { preHandler: requireSubscriber },
    async (request) => services.usage.summary(subscriberPrincipal(request)),
  );
  ...
  app.get(
    "/api/v1/subscriber/sessions",
    { preHandler: requireSubscriber },
    async (request) => services.sessions.list(subscriberPrincipal(request)),
  );

  app.get(
    "/api/v1/subscriber/devices",
    { preHandler: requireSubscriber },
    async (request) => services.devices.list(subscriberPrincipal(request)),
  );
  ...
  app.put<{ Params: { id: string }; Body: { selection: string } }>(
    "/api/v1/subscriber/devices/:id/speed",
    {
      preHandler: requireSubscriber,
      config: {
        rateLimit: {
  ...
  app.get(
    "/api/v1/subscriber/recharges",
    { preHandler: requireSubscriber },
    async (request) => services.recharges.list(subscriberPrincipal(request)),
  );
  ...
  app.patch(
    "/api/v1/subscriber/notifications/read-all",
    { preHandler: requireSubscriber },
    async (request) =>
      services.notifications.markAllRead(subscriberPrincipal(request)),
  );

  app.get(
    "/api/v1/subscriber/speed",
    { preHandler: requireSubscriber },
    async (request) => services.speed.get(subscriberPrincipal(request)),
  );
  app.put<{ Body: { selection: string } }>(
    "/api/v1/subscriber/speed",
    {
      preHandler: requireSubscriber,
      config: {
    ...
    async (request) =>
      services.speed.set(subscriberPrincipal(request), request.body.selection),
  );

  app.get(
    "/api/v1/subscriber/connection-limit",
    { preHandler: requireSubscriber },
    async (request) =>
      services.connectionLimits.get(subscriberPrincipal(request)),
  );
  app.put<{ Body: { limit: number } }>(
    "/api/v1/subscriber/connection-limit",
    {
      preHandler: requireSubscriber,
      config: {
