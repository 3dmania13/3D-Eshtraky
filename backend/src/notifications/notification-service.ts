...
export class NotificationService {
  ...
  async list(principal: SubscriberPrincipal) {
    ...
    return (
      await this.repository.getNotifications(principal.username, 100)
    ).map((item) => ({
      id: item.id,
      type: item.type,
      title: item.title,
  ...
  async markAllRead(principal: SubscriberPrincipal) {
  ...
  async registerPushToken(principal: SubscriberPrincipal, token: string) {
  ...
  checkExpiryWarnings(dashboard: DashboardRecord): DerivedNotification[] {
    const days = this.subscriptions.daysRemaining(
      dashboard.expiresAt,
      this.now(),
    );
  ...
  checkQuotaWarnings(dashboard: DashboardRecord): DerivedNotification[] {
    const result = this.quota.calculate(
      dashboard.totalBytes,
      dashboard.usedBytes,
    );
    if (result.totalBytes === 0n) return [];
    ...
    if (result.usagePercentage < 75) return [];
