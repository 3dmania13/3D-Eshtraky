...
import type { LiveSpeedApplier, LiveSpeedResult } from './live-speed-service.js';
...
export class SpeedService {
  ...
  constructor(private readonly repository: SubscriberRepository, private readonly live?: LiveSpeedApplier) {}
  ...
  async set(principal: SubscriberPrincipal, selection: string) {
    const rate = selections[selection as keyof typeof selections];
    if (!rate) throw new AppError(400, 'INVALID_SPEED', 'السرعة المختارة غير صحيحة.');
    ...
    try {
      ...
      let live: LiveSpeedResult = { status: 'pending', active_sessions: 0, updated_sessions: 0 };
      try {
        if (this.live) live = await this.live.apply(principal.username, rate);
